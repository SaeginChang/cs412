...
class RegistrationView(CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # if we received an HTTP POST
        if self.request.POST:

            # we handle it
            print(f"RegistrationView.dispatch: self.request.POST={self.request.POST}")

            #reconstruct the UserCreate Form from the POST data
            ...
            # note for mini_fb: attach the FK user to the Profie from instance

            # return a response
            return redirect(reverse('show_all'))

        # let Create View.dispatch handle the HTTP GET request
        return super().dispatch(request, *args, **kwargs)